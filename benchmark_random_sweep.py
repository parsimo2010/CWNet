...
def main():
    ...
    parser.add_argument("--window-sec", type=float, default=8.0,
                        help="Decoder window size in seconds")
    parser.add_argument("--stride-sec", type=float, default=4.0,
                        help="Decoder stride in seconds")
    ...
    win_samples = int(args.window_sec * dec.sample_rate)
    stride_samples = int(args.stride_sec * dec.sample_rate)
    ...
    print(f"Running {n} random evaluations (full scenario, greedy decode)")
    ...
    for i in range(n):
        ...
        if audio_samples <= win_samples:
            ...
            n_windows = 0
            pos = 0
            while pos < audio_samples:
                chunk_len = min(win_samples, audio_samples - pos)
                if chunk_len >= win_samples // 4:
                    n_windows += 1
                pos += stride_samples
