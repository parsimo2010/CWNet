"""
inference.py — Inference for the Hybrid Event Transformer decoder.

Provides sliding-window bidirectional decoding (primary mode) and
whole-file decoding. Supports greedy CTC, beam search, and
LM-augmented beam search.

Uses HybridFeaturizer (17-dim) instead of EnhancedFeaturizer (10-dim),
combining base features with Bayesian timing posteriors from the
reference decoder's timing model.

Usage (Python API):
    from hybrid_decoder.inference import HybridTransformerDecoder

    dec = HybridTransformerDecoder("checkpoints_hybrid/best_model.pt")
...
def _load_hybrid_checkpoint(
    ...
    """Load Hybrid Event Transformer checkpoint.
    ...
    model_cfg = EventTransformerConfig(
        in_features=mc.get("in_features", 17),
...
def _extract_all_events_and_features(
    ...
    Runs the MorseEventExtractor and HybridFeaturizer once over the entire
    audio stream so that adaptive thresholds, running statistics, and
    Bayesian timing posteriors build up properly across the full duration.

    Returns:
        features: (T, 17) float32 feature array.
...
class HybridTransformerDecoder:
    """Bidirectional sliding-window decoder for the Hybrid Event Transformer.

    Processes audio in overlapping windows, runs bidirectional attention
    on each window, and stitches results. Uses HybridFeaturizer (17-dim)
    which combines EnhancedFeaturizer features with Bayesian timing
    posteriors from the reference decoder's timing model.

    Args:
        checkpoint: Path to Hybrid Event Transformer checkpoint.
    ...
    def __init__(
        ...
        self._model, self._model_cfg, self._feature_cfg, self.sample_rate = (
            _load_hybrid_checkpoint(checkpoint, self.device)
        )
    ...
    def decode_audio(self, audio: np.ndarray) -> str:
        ...
        audio so that adaptive thresholds, running statistics, and Bayesian
        timing posteriors accumulate properly. Only the transformer sees
        windowed slices of the resulting feature sequence.
    ...
    def decode_events(self, features: np.ndarray) -> str:
        """Decode pre-computed features (T, 17) array.

        Useful for direct event generation path (no audio).

        Args:
            features: (T, 17) float32 feature array from HybridFeaturizer.
...
def main():
    parser = argparse.ArgumentParser(
        description="Decode Morse code audio with Hybrid Event Transformer",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--checkpoint", required=True, metavar="PATH",
                        help="Path to Hybrid Event Transformer checkpoint")
    ...
    print(f"[hybrid] window={dec.window_size}s stride={dec.stride}s "
