...
def apply_rope(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    ...
    cos, sin : Tensor, shape (seq_len, dim//2) or broadcastable
    ...
    # Trim cos/sin to actual sequence length
    seq_len = x.shape[-2]
    cos = cos[:seq_len].unsqueeze(0)  # (1, seq_len, dim//2)
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor) -> tuple[Tensor, Tensor]:
        ...
        """
        return apply_rope(q, self.cos, self.sin), apply_rope(k, self.cos, self.sin)
